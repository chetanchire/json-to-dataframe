...
summary_data <- data.frame(carrierID = character(), data_type = character(),
  ...
  if (length(json_data) == 18) {
    ...
    if (length(json_data$`Cycle Data`) > 0) {
      ...
      for (k in seq_len(length(json_data$`Cycle Data`))) {
        ...
        if (length(json_data$`Run Details`) >=
          ...
          summ_data$rig_Name[b] <- json_data$`Run Details`[[json_data$`Cycle Data`[[k]]$run_num]]$`Rig Name`
          summ_data$run_name[b] <- json_data$`Run Details`[[json_data$`Cycle Data`[[k]]$run_num]]$`Run Name`
          summ_data$additional_notes[b] <- json_data$`Run Details`[[json_data$`Cycle Data`[[k]]$run_num]]$`Run Completion Details`$`Additional Notes`
        } else {
    ...
    if (length(json_data$`Non-Detect Cycle Data`) > 0) {
      ...
      for (i in seq_len(length(json_data$`Non-Detect Cycle Data`))) {
        ...
        summ_data$experiment_name[b] <- json_data$`Non-Detect Cycle Data`[[i]]$experiment_name
        summ_data$script_name[b] <- json_data$`Non-Detect Cycle Data`[[i]]$script_name
        summ_data$avg_final_wash_thru_dp[b] <- json_data$`Non-Detect Cycle Data`[[i]]$cycle_metrics[[1]]

        if (length(json_data$`Run Details`) >= json_data$`Non-Detect Cycle Data`[[i]]$run_num) {
          summ_data$rig_Name[b] <- json_data$`Run Details`[[json_data$`Non-Detect Cycle Data`[[i]]$run_num]]$`Rig Name`
          summ_data$run_name[b] <- json_data$`Run Details`[[json_data$`Non-Detect Cycle Data`[[i]]$run_num]]$`Run Name`
          summ_data$additional_notes[b] <- json_data$`Run Details`[[json_data$`Non-Detect Cycle Data`[[i]]$run_num]]$`Run Completion Details`$`Additional Notes`
        } else {
          summ_data$rig_Name[b] <- "DNF"
          summ_data$run_name[b] <- "DNF"
          summ_data$additional_notes[b] <- "DNF"
    ...
    for (j in seq_len(length(json_data$`Lane Content`))) {
      summ_data[, 11 + 2 * j] <- json_data$`Lane Content`[[j]]$sample
      summ_data[, 12 + 2 * j] <- json_data$`Lane Content`[[j]]$concentration
    }
